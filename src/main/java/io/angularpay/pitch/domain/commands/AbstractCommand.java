...
import io.angularpay.pitch.exceptions.ValidationException;
import io.angularpay.pitch.models.AccessControl;
...
import static io.angularpay.pitch.helpers.Helper.*;
...
public abstract class AbstractCommand<T extends AccessControl, R> {
    ...
    public R execute(T request) {
        try {
            ...
            if (this instanceof UpdatesPublisherCommand && response instanceof PitchRequestSupplier) {
                log.info("publishing {} update to REDIS => message payload:  {}", this.name, responseText);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UpdatesPublisherCommand)this).publishUpdates((PitchRequestSupplier)response);
                });
            }
            if (this instanceof TTLPublisherCommand && response instanceof PitchRequestSupplier) {
                log.info("publishing {} TTL to REDIS", this.name);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((TTLPublisherCommand)this).publishTTL((PitchRequestSupplier)response);
                });
            }
            if (this instanceof UserNotificationsPublisherCommand && response instanceof PitchRequestSupplier) {
                log.info("publishing {} User Notification to REDIS", this.name);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UserNotificationsPublisherCommand)this).publishUserNotification((PitchRequestSupplier)response);
                });
            }
